...
sudo apt-get -y install swig4.0 python3-dev python3-pip build-essential pkg-config libssl-dev libffi-dev libhwloc-dev libboost-dev cmake git libleveldb-dev ntp
...
cd qrandomx
sed -i 's/jit_compiler_x86/jit_compiler_a64/g' CMakeLists.txt
